...
class UploadForm extends Model
{
    ...
    protected function uploadFile($file)
    {
        ...
        $sizeCompressed = $this->compressImage($file, $dist);
        $size = $sizeCompressed ? $sizeCompressed : $size;
    ...
    protected function compressImage($file, $dist)
    {
        /* 
         * Additional image compression by Louis 
         * It replaced the saved file by compressed 1
         * @date: 20230718
        */
        if ( in_array($file->extension, ['jpg', 'jpeg', 'png', 'gif', 'webp', 'raw'])) {
            if ($file->size >100000) {
                Image::getImagine()
                ->open($dist)
                ->save($dist, ['quality' => 20]); // Adjust the quality as needed (0-100)
            } else {
                Image::getImagine()
                ->open($dist)
                ->save($dist, ['quality' => 50]); // Adjust the quality as needed (0-100)
            }

            clearstatcache();
            // $optimizerChain = OptimizerChainFactory::create();
            // $optimizerChain->optimize($dist);
            $size = filesize($dist);
            return $size;
        }
